use super::super::super::application::ports::user_repository::UserRepository;
...
impl UserRepository for MongoUserRepository {
    async fn save(&self, user:User) -> Result<User, String>{
        ...
        self.collection.insert_one(doc).await.map_err(|e| e.to_string())?;
        Ok(user)
    } 
    async fn find_by_id(&self, id: ID) -> Result<User,String>{
        let doc_option = self.collection.find_one(doc!{"id": id.value()}).await.map_err(|e| e.to_string())?;
        match doc_option {
